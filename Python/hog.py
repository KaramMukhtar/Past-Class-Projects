    ...
    def commentary (score0, score1):
        ...
        if (score - previous_score) > previous_high:
            if score - previous_score == 1:
                ...
                label = "points!"
            print (score - previous_score, label,"That's the biggest gain yet for Player", who)
            return announce_highest(who, score - previous_score, score)
        else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    highest_dice = 1
    highest_average = 0
    avg_fun = make_averaged (roll_dice, num_samples)
    while num_rolls <= 10:
        avg = avg_fun(num_rolls, dice)
        if avg > highest_average:
            highest_average = avg
            highest_dice = num_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    if is_swap(opponent_score, free_bacon(opponent_score) + score) and (free_bacon(opponent_score) + score) < opponent_score:
        return 0
    else:
